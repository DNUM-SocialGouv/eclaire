...
export class IngestPipelineImport extends IngestPipeline {
  ...
  async runWithProgress(onProgress?: (p: number) => void): Promise<void> {
    ...
    this.filePath = await exporter.exportSheets(
      [
        { name: 'ETUDES DM (2017-745)', columns: DM_COLUMNS, data: dm745Data },
        { name: 'ETUDES DM-DIV (2017-746)', columns: DM_COLUMNS, data: dm746Data },
        { name: 'ETUDES JARDE', columns: JARDE_COLUMNS, data: jardeData },
        { name: 'ETUDES CTIS (2014-536)', columns: CTIS_COLUMNS, data: ctisData },
      ],
      (rowsProcessed) => {
